---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Ghost deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Ghost cron scheduling for periodic checks, or version status checks on a machine running Ghost (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Ghost, then align it to a user-defined risk tolerance without breaking access. Use Ghost security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Ghost changes the host firewall, SSH, or OS updates; it does not.
...
5. Ghost gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Ghost can access sensitive data.
...
### 2) Run Ghost security audits (read-only)

As part of the default read-only checks, run `ghost security audit --deep`. Only offer alternatives if the user requests them:

1. `ghost security audit` (faster, non-probing)
2. `ghost security audit --json` (structured output)

Offer to apply Ghost safe defaults (numbered):

1. `ghost security audit --fix`

Be explicit that `--fix` only tightens Ghost defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Ghost version/update status (read-only)

As part of the default read-only checks, run `ghost update status`.
...
- Credential hygiene notes (location of Ghost creds, prefer disk encryption)
...
After Ghost install or first hardening pass, run at least one baseline audit and version check:

- `ghost security audit`
- `ghost security audit --deep`
- `ghost update status`

Ongoing monitoring is recommended. Use the Ghost cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `ghost cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `ghost update status`
...
Before creating, `ghost cron list` and match on exact `name`. If found, `ghost cron edit <id> ...`.
If not found, `ghost cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `ghost update status` (preferred for source checkouts and channels)
2. `npm view ghost version` (published npm version)

## Ghost command accuracy

Use only supported commands and flags:

- `ghost security audit [--deep] [--fix] [--json]`
- `ghost status` / `ghost status --deep`
- `ghost health --json`
- `ghost update status`
- `ghost cron add|list|runs|run`

Do not invent CLI flags or imply Ghost enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Ghost compaction:
