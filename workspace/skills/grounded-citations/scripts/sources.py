...
Ledger path resolution (first wins):
  --ledger PATH
  $GHOST_CITATION_LEDGER
  $GHOST_HOME/cache/citations/ledger.json
"""
...
from _ghost_home import get_ghost_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("GHOST_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_ghost_home() / "cache" / "citations" / "ledger.json"
