...
This is an optional third-party workflow, not a Ghost core capability.

Important expectations:

- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Ghost native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Ghost native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Ghost native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Ghost usage patterns
...
- Do not silently choose Parallel when Ghost native tools are already sufficient.
