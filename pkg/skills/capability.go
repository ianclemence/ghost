...
var capabilityRegistry = map[string]Capability{
	...
	"email":       {ID: "email.read", Skill: "email", AllowedTools: []string{"read_file", "exec"}, Primary: "himalaya via exec + Gmail/Outlook connected apps", Fallback: "none", MaxAttempts: 2, Timeout: 15 * time.Second, NetworkRequired: true, Risk: "consequential"},
