	...
	if channels, ok := getMap(data, "channels"); ok {
		for name, val := range channels {
			...
			case "email":
				...
				if v, ok := getString(cMap, "smtp_host"); ok {
				...
				if v, ok := getString(cMap, "from"); ok {
					cfg.Channels.Email.From = v
				}
				if v, ok := getString(cMap, "to"); ok {
					cfg.Channels.Email.To = v
				}
			}
		}
	}
...
func MergeConfig(existing, incoming *config.Config) *config.Config {
	...
	if !existing.Channels.Email.Enabled && incoming.Channels.Email.Enabled {
		existing.Channels.Email = incoming.Channels.Email
	}
