...
func (m dashboardModel) View() string {
	...
	case ModeWorkspace:
		body = m.renderWorkspaceIntelligence(m.width, availableHeight)
	default:
...
func (m dashboardModel) renderHeader() string {
	...
	statusText := "● LIVE"
	if m.isRefreshing {
		statusColor, statusText = themeAccent, "⟳ SYNC"
	}
	if m.lastError != "" && time.Since(m.lastErrorAt) < 30*time.Second {
		statusColor, statusText = themeError, "● ERR"
	}
...
func (m dashboardModel) renderChannelItem(name string, selected bool, width int) string {
	...
	if ch.Running {
		statusColor, statusIcon = themeSuccess, "●"
	} else if ch.Enabled {
		statusColor, statusIcon = themeError, "●"
	}
...
func (m dashboardModel) renderChannelDetails(name string, width int) string {
	...
	if ch.LastSendErr != "" {
		...
		if strings.Contains(strings.ToLower(ch.LastSendErr), "rate") {
			category = "Rate Limit"
		}
		if strings.Contains(strings.ToLower(ch.LastSendErr), "auth") {
	...
	if ch.Fatal {
		lines = append(lines, lipgloss.NewStyle().Foreground(themeWarning).Render("  ⚠ Fatal / Suspended"))
	}
...
func (m dashboardModel) renderEventLog(width, maxHeight int) string {
	if len(m.trace.Events) == 0 {
		return lipgloss.NewStyle().Foreground(cTextMuted).Render("  Listening for traces...")
	}
	var lines []string
...
func (m dashboardModel) renderEventLine(evt traceEvent, width int, isLatest bool) string {
	...
	if strings.Contains(s, "fail") || strings.Contains(s, "err") {
		icon, color = "✕", themeError
	}
	if strings.Contains(s, "queue") || strings.Contains(s, "process") {
		icon, color = "◎", themeWarning
	}
...
func (m dashboardModel) renderFooter() string {
	...
	if m.mode == ModeChat {
		controls = "Esc Close Chat  •  ↵ Transmit"
...
func (m dashboardModel) renderSanityChecks() string {
	...
	for _, c := range []struct {
		n string
		v bool
	}{{"API Socket", m.sanity.APIReachable}} {
		icon, color := "●", themeSuccess
		if !c.v {
			icon, color = "●", themeError
		}
...
func reconnectCmd(client *operatorClient, channel string) tea.Cmd {
	return func() tea.Msg { return reconnectMsg{channel: channel, err: client.reconnect(channel)} }
}
		...
		for scanner.Scan() {
			if strings.HasPrefix(scanner.Text(), "data: ") {
				...
				if json.Unmarshal([]byte(d), &evt) == nil && evt["type"] == "tool_status" {
					...
					var t string
					if json.Unmarshal([]byte(d), &t) == nil {
						reply += t
					}
				}
			}
...
func (c *operatorClient) fetchDoctorAndChannels() (doctorPayload, map[string]channelHealth, error) {
	...
	if err := c.get("/v1/doctor", &res); err != nil {
...
func (c *operatorClient) fetchTrace(reqID string) (tracePayload, error) {
	var res tracePayload
	return res, c.get("/v1/traces?request_id="+url.QueryEscape(reqID), &res)
}
func (c *operatorClient) fetchTelemetry() (telemetryPayload, error) {
	var res telemetryPayload
	return res, c.get("/v1/telemetry", &res)
}
